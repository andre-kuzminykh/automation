#!/usr/bin/env bash
# Parallel AI-maturity-test head generation (35 videos, chunked, batched push).
#
# Usage (on human-1, venv active, HEDRA_API_KEY exported):
#   git pull --rebase origin claude/setup-gcloud-video-service-XKVf0
#   PAR=4 bash mt/run_parallel.sh            # all slides 1..35, 4 in parallel
#   bash mt/run_parallel.sh 10 20            # subset
set -u
...
render_one() {
  local i="$1"
  echo "---- start slide $i ----"
  python3 -m l7.service.generate --lecture mt \
...
push_chunk() {
  ...
  git add mt/videos/ mt/data/manifest.json 2>/dev/null
  git checkout -- intro/data/manifest.json l1/data/manifest.json l2/data/manifest.json 2>/dev/null || true
  if git diff --cached --quiet; then
    echo "[push] nothing to commit ($label)"
    return
  fi
  git commit -m "feat(mt): hedra videos for $label" >/dev/null
